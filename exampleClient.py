...
target_x_slider = env.sim.physics_client.addUserDebugParameter("target_x", -1, 1, 0)
target_y_slider = env.sim.physics_client.addUserDebugParameter("target_y", -1, 1, 0)
target_z_slider = env.sim.physics_client.addUserDebugParameter("target_z", -1, 1, 0)
...
speed = 1
while True:
    keys = env.sim.physics_client.getKeyboardEvents()
    action = np.zeros(7)
    if not ord(' ') in keys:
      action = solveSmoothHybrid(robot, model, obs)
    obs, reward, done, info = env.step(action)
    env.render()
    x = env.sim.physics_client.readUserDebugParameter(target_x_slider) #math.cos(time.time() * speed) * size
    y = env.sim.physics_client.readUserDebugParameter(target_y_slider) #math.sin(time.time() * speed) * size
    z = env.sim.physics_client.readUserDebugParameter(target_z_slider) #offset
    goal = np.array([x, y, z])
    env.set_goal(goal)
    if done:
      obs = env.reset()