...
for i in range(0, testCount):
    ...
    current_pos, target_pos = obs["achieved_goal"], obs["desired_goal"]
    d = distance(current_pos, target_pos)
    DLS_results.append(d < 0.02)

#Hybrid with 0.1 distance threshold
hybrid01_results = []
SWAP_THRESHOLD = 0.1
for i in range(0, testCount):
    env.seed(i)
    ...
    current_pos, target_pos = obs["achieved_goal"], obs["desired_goal"]
    d = distance(current_pos, target_pos)
    hybrid01_results.append(d < 0.02)

#Hybrid with 0.2 distance threshold
hybrid02_results = []
SWAP_THRESHOLD = 0.2
for i in range(0, testCount):
    env.seed(i)
    ...
    current_pos, target_pos = obs["achieved_goal"], obs["desired_goal"]
    d = distance(current_pos, target_pos)
    hybrid02_results.append(d < 0.02)

#Smooth hybrid
smooth_hybrid_results = []
for i in range(0, testCount):
    env.seed(i)
    obs = env.reset()
    for it in range(0, 300):
    ...
    current_pos, target_pos = obs["achieved_goal"], obs["desired_goal"]
    d = distance(current_pos, target_pos)
    smooth_hybrid_results.append(d < 0.02)

i = 0
for results in zip(DLS_results, hybrid01_results, hybrid02_results, smooth_hybrid_results):
    print(i, *results, sep=',')
    i = i + 1