...
class PyBulletRobot(ABC):
    ...
    def __init__(
        ...
        self.current_seed = self.seed(None)
    ...
    def get_link_rotation(self, link: int) -> np.ndarray:
        """Returns the rotation of a link as (rx, ry, rz)

        Args:
            link (int): The link index.

        Returns:
            np.ndarray: Rotation as (rx, ry, rz)
        """
    ...
    def get_joint_angles(self) -> np.array:
        """Get the current joints angles of a body.

        Returns:
            current_angles (list): Joint angles of the joint_indices joints.
        """
        current_angles = []
        for angle in self.joint_indices:
            current_angles.append(self.sim.get_joint_angle(self.body_name, joint=angle))
...
class RobotTaskEnv(gym.GoalEnv):
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, Dict[str, Any]]:
        ...
        self.sim.step()
        obs = self._get_obs()
        done = False #self.task.is_success(obs["achieved_goal"], self.task.get_goal())
    ...
    def seed(self, seed: Optional[int] = None) -> int:
        """Setup the seed."""
        return self.task.seed(seed), self.robot.seed(seed)
